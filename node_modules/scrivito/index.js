...
function initializeGlobalState(ui) {
    if (!ui) {
        ...
        sharedCmsState = globalState.subState('sharedCms');
        cmsState = globalState.subState('localCms');
        ...
        sharedCmsState = ui.sharedCmsState();
        cmsState = appState.subState('localCms');
    }
    setupSubscribers();
}
...
function subscribeEntireStateForTestPurposes(listener) {
    ...
    var entireTree = globalState;
    subscriber.subscribeChanges(trackStateAccess(function () { return entireTree.get(); }).accessedState);
...
function trackStateAccess(fn) {
    return stateController.trackStateAccess(fn);
}
...
var unsubscribeStateController;
function setupSubscribers() {
    unsubscribeStateController = stateController.subscribe(function () {
        syncSubscribers.notify();
        asyncSubscribers.notify();
    });
}
/** Unsubscribe all subscribers from the global state.
 * This is done when the SDK shuts down.
 * It removes the subscription to the UI's globalState.
 */
function terminateLocalState() {
    if (unsubscribeStateController) {
        unsubscribeStateController();
    }
    // remove all local state to allow GC (otherwise memory leak!)
    appState.clear();
}
...
function waitFor(observableFunction) {
    return new ScrivitoPromise(function (resolve) {
        var subscription = observe(observableFunction).subscribe(function (value) {
...
function load(loadableFunction) {
    ...
    checkLoad.apply(void 0, [loadableFunction].concat(excessArgs));
    var deferred = new common_deferred();
    ...
    Object.keys(description).forEach(function (methodName) {
        ...
        if (methodType === GET) {
            adapterClient[methodName] = function () {
                ...
                var data = new loadable_data({
                    state: state,
                    stream: connection.stream.apply(connection, [methodNameAsGet].concat(params)),
                });
                return data.get();
            };
        }
        if (methodType === SEND) {
            adapterClient[methodName] = function () {
                ...
                return connection.send.apply(connection, [methodName].concat(params));
            };
        }
        if (methodType === STREAM) {
            adapterClient[methodName] = function () {
                ...
                return connection.stream.apply(connection, [methodName].concat(params));
            };
        }
    });
    ...
    return {
        stream: function (methodName) {
            ...
            return observeAndLoad(function () {
                ...
                return { valueFromClient: method.call.apply(method, [client].concat(params)) };
            })
        ...
        send: function (methodName) {
            ...
            var promise = new ScrivitoPromise(function (resolve) {
                waitFor(currentClient).then(function (client) {
                    var method = client[methodName];
                    resolve(method.call.apply(method, [client].concat(params)));
                });
            });
...
function createAdapterConnection(description, adapter) {
    return {
        stream: function (methodName) {
            ...
            var methodType = description[methodName];
            if (methodType === GET) {
                return observeAndLoad(function () { return method.call.apply(method, [adapter].concat(params)); })
                    .filter(function (observed) { return observed.allDataLoaded; })
                    .map(function (observed) { return observed.result; });
            }
            if (methodType === STREAM) {
                var result = method.call.apply(method, [adapter].concat(params));
        ...
        send: function (methodName) {
            ...
            var method = adapter[methodName];
            var promise = new ScrivitoPromise(function (resolve) {
                nextTick(function () { return resolve(method.call.apply(method, [adapter].concat(params))); });
            });
            // Promise<T> is assignable to Promisify<T>, but TS does not infer that
            return promise;
        },
...
var adapter_message_client_AdapterMessageClient = /** @class */ (function () {
    ...
    AdapterMessageClient.prototype.send = function (methodName) {
        ...
        var resultPromise = this.incomingChannels
            ...
            if (!message)
                throw new AdapterDisconnectedError();
...
var AdapterDisconnectedError = /** @class */ (function (_super) {
    adapter_message_client_extends(AdapterDisconnectedError, _super);
    function AdapterDisconnectedError() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    return AdapterDisconnectedError;
}(ScrivitoError));
...
var adapter_message_server_AdapterMessageServer = /** @class */ (function () {
    ...
    AdapterMessageServer.prototype.adapterStreamForInvocation = function (invocation) {
        ...
        if (isMatchingAdapterInvocation(this.description, GET, invocation)) {
            return (_a = this.adapterConnection).stream.apply(_a, [invocation.methodName].concat(invocation.params));
        }
        if (isMatchingAdapterInvocation(this.description, STREAM, invocation)) {
            return (_b = this.adapterConnection).stream.apply(_b, [invocation.methodName].concat(invocation.params));
        }
    };
    AdapterMessageServer.prototype.handleSendMessage = function (message) {
        ...
        (_a = this.adapterConnection).send.apply(_a, [invocation.methodName].concat(invocation.params)).then(function (data) {
...
function establishUiConnection(ui) {
    return createAdapterClient(uiAdapterDescription, createAdapterMessageClient(linkViaPort(ui.uiAdapterPort())));
}
...
var basic_link_BasicLink = /** @class */ (function () {
    ...
    BasicLink.prototype.copy = function (attributes) {
        ...
        var newAttributes = basic_link_assign({}, this.attributes, attributes);
...
var future_binary_FutureBinary = /** @class */ (function () {
    ...
    FutureBinary.prototype.into = function (target) {
        ...
        checkInto.apply(void 0, [target].concat(excessArgs));
...
function retrieveObj(workspaceId, id) {
    return getBatchRetrieval(workspaceId)
        .retrieve(id)
        .then(function (value) {
        if (value) {
            return value;
...
function setContentStateId(workspaceId, contentStateId) {
    if (contentUpdateHandler)
        return;
    getState(workspaceId).set(contentStateId);
}
// For test purpose only
...
var obj_replication_pool_ObjReplicationPool = /** @class */ (function () {
    function ObjReplicationPool(strategy) {
        this.strategy = strategy;
        this.replicationCache = new ReplicationCache(function (workspaceId, objId) {
            if (!strategy)
                throw new InternalError();
            return new strategy(workspaceId, objId);
        });
        this.disabled = false;
...
// the initial objReplicationPool is just a placeholder, to be replaced
var objReplicationPool = new obj_replication_pool_ObjReplicationPool(undefined);
...
function useReplicationStrategy(strategy) {
    replaceObjReplicationPool(new obj_replication_pool_ObjReplicationPool(strategy));
}
...
function setObjDataNonexistent(workspaceId, objId) {
    setObjData(workspaceId, objId, { _deleted: objId });
}
...
var obj_backend_replication_ObjBackendReplication =
/*#__PURE__*/
function () {
  function ObjBackendReplication(workspaceId, objId) {
    var _this = this;

    _classCallCheck(this, ObjBackendReplication);
  ...
  _createClass(ObjBackendReplication, [{
    key: "start",
    value: function start() {
      ...
      retrieveObj(this._workspaceId, this._objId).then(function (data) {
        addBatchUpdate(function () {
          _this2.notifyBackendState(data);
        });
      });
    }
  }, {
    ...
    value: function notifyLocalState(localState) {
      ...
      if (external_underscore_default.a.isEmpty(obj_backend_replication_diff(this._localState, localState))) return;
      ...
      if (this._backendState === undefined) this._backendState = null;
      ...
      if (!external_underscore_default.a.isEmpty(obj_backend_replication_diff(this._backendState, this._localState))) {
        if (!this._replicationActive) {
          if (!this._scheduledReplication) {
            ...
            nextTick(function () {
              return _this3._performThrottledReplication();
            });
          }
    ...
    value: function _performReplication() {
      ...
      this._replicatePatchToBackend(patch).then(function (backendState) {
        _this4._handleBackendUpdate(localState, backendState);

        _this4._currentRequestDeferred.resolve(_this4._objId);

        _this4._currentRequestDeferred = null;
        _this4._replicationActive = false;

        _this4._startReplication();
      }, function (error) {
        _this4._currentRequestDeferred.reject(error);
    ...
    value: function _replicatePatchToBackend(patch) {
      ...
      var path = "workspaces/".concat(this._workspaceId, "/objs/").concat(this._objId);
...
var obj_id_query_batch_loadableCollection = new loadable_collection_LoadableCollection({
    ...
    loader: loader,
    invalidation: function (_a) {
        var workspaceId = _a[0], _params = _a[1], _index = _a[2];
        return loadableWithDefault(undefined, function () { return getContentStateId(workspaceId); }) || '';
    },
});
...
function loader(_a, _b, batchSize) {
    var workspaceId = _a[0], params = _a[1], index = _a[2];
    ...
    return fetchContinuation(workspaceId, params, batchSize, index).then(function (continuation) {
        ...
        return cmsRetrieval
            ...
            push(function () {
                return includedObjs.forEach(function (objJson) {
                    objReplicationPool
                        .get(workspaceId, objJson._id)
                        .notifyBackendState(objJson);
                });
            });
            var includedObjIds = includedObjs.map(function (obj) { return obj._id; });
            var idsToPreload = response.results.filter(function (resultId) { return includedObjIds.indexOf(resultId) < 0; });
            obj_id_query_batch_preloadObjData(workspaceId, idsToPreload);
            return {
...
function obj_id_query_batch_preloadObjData(workspaceId, ids) {
    ids.forEach(function (id) { return preloadObjData(workspaceId, id); });
}
...
function getObjFieldDiffs(workspaceId, objId) {
    return getLoadableData(workspaceId, objId).get() || {};
}
...
var binary_Binary = /** @class */ (function () {
    ...
    Binary.upload = function (source, options) {
        ...
        checkUpload.apply(void 0, [source, options].concat(excessArgs));
...
var basic_obj_BasicObj = /** @class */ (function (_super) {
    ...
    BasicObj.addChildWithSerializedAttributes = function (parentPath, serializedAttributes) {
        ...
        return this.createWithSerializedAttributes(current_workspace_currentWorkspaceId(), basic_obj_assign({}, serializedAttributes, { _id: objId, _path: parentPath + "/" + objId }));
    };
    BasicObj.createWithSerializedAttributes = function (workspaceId, serializedAttributes, attributes) {
        if (!attributes) {
            var _a = extractWidgetlistValues(serializedAttributes), serializedAttributesWithoutWidgetlists = _a[0], widgetlistAttributes = _a[1];
    ...
    BasicObj.prototype.copyAsync = function (copyOptions) {
        ...
        return this._copyAttributes().then(function (copiedAttributes) {
            var serializedAttributes = basic_obj_assign({}, copiedAttributes, copyOptions);
    ...
    BasicObj.prototype._copyAttributes = function () {
        ...
        var serializedAttributes = basic_obj_assign({}, customSerializedAttributes, { _id: objId, _obj_class: this.objClass(), _restriction: this.restriction() });
        if (this.path()) {
...
var basic_obj_search_BasicObjSearch = /** @class */ (function () {
    ...
    BasicObjSearch.prototype.take = function (count) {
        ...
        checkTakeArguments.apply(void 0, [count].concat(excessArgs));
...
function browser_location_get() {
    var location = getHistory().location;
    return "" + location.pathname + location.search + location.hash;
}
function getIncludingOrigin() {
    return "" + window_proxy_location().origin + browser_location_get();
}
...
function normalizeAttributeDefinitionOptions(options) {
    ...
    return schema_assign({}, otherOptions, { validClasses: validClasses });
}
  ...
  function () {
    ...
    attribute_content_factory_createClass(AttributeContent, [{
      ...
      value: function get(attributeName) {
        ...
        if (isSystemAttribute(attributeName)) {
          throw new ArgumentError("Attribute name \"".concat(attributeName, "\" is not a valid custom attribute name."));
        }
...
function ObjSearchFactory(registry) {
    ...
    var ObjSearch = /** @class */ (function () {
        ...
        ObjSearch.prototype.facet = function (attribute, options) {
            ...
            return basicFacets.map(function (facetValue) { return new obj_facet_value(registry, facetValue); });
        };
        // public API
        ...
        ObjSearch.prototype.take = function (count) {
            ...
            return basicObjs.map(function (obj) { return wrap_in_app_class_wrapInAppClass(registry, obj); });
        };
        // public API
                ...
                return {
                    next: function () {
                        ...
                        return {
                            ...
                            value: wrap_in_app_class_wrapInAppClass(registry, iteratorResult.value),
                        };
                    },
                };
            };
...
function WidgetFactory(registry) {
  ...
  function (_AttributeContentFact) {
    ...
    function Widget() {
      ...
      var schema = realm_schema.forInstance(widget_factory_assertThisInitialized(_this));
    ...
    widget_factory_createClass(Widget, [{
      ...
      value: function container() {
        ...
        return wrap_in_app_class_wrapInAppClass(registry, container);
      }
    }, {
...
var realm_Realm = /** @class */ (function () {
    ...
    Realm.prototype.provideObjClass = function (name, definition) {
        ...
        checkProvideObjClass.apply(void 0, [name, definition].concat(excessArgs));
    ...
    Realm.prototype.provideWidgetClass = function (name, definition) {
        ...
        checkProvideWidgetClass.apply(void 0, [name, definition].concat(excessArgs));
    ...
    Realm.prototype.createObjClass = function (definition) {
        ...
        checkCreateObjClass.apply(void 0, [definition].concat(excessArgs));
    ...
    Realm.prototype.createWidgetClass = function (definition) {
        ...
        checkCreateWidgetClass.apply(void 0, [definition].concat(excessArgs));
...
function provideEditingConfig(className, editingConfig) {
    ...
    checkProvideEditingConfig.apply(void 0, [className, editingConfig].concat(excessArgs));
...
function getCurrentPageState() {
    var state = loadWithDefault(undefined, function () { return loadablePage().get(); });
...
function recognizeLocation() {
    var location = browser_location_get();
...
function stateForRouteAndLocation(_a) {
    ...
    if (!isBinaryBasicObj(obj)) {
        var _b = route.queryParameters, _scrivito_workspace_id = _b._scrivito_workspace_id, _scrivito_editing = _b._scrivito_editing, _scrivito_comparison = _b._scrivito_comparison, queryParameters = current_page_state_rest(_b, ["_scrivito_workspace_id", "_scrivito_editing", "_scrivito_comparison"]);
        return {
            type: 'page',
            objId: obj.id(),
            queryParameters: queryParameters,
...
function onCurrentPageChange(callback) {
    var lastState;
    return observe(function () { return capture(getCurrentPageState).result; }).subscribe(function (currentState) {
...
function switchToCanonicalUrl(state) {
    ...
    load(function () {
        if (browser_location_get() !== location) {
            return;
        }
        ...
        var browserLocation = browser_location_get();
...
function initializeEditingContextFromLocation() {
    ...
    var workspaceId = params._scrivito_workspace_id || 'published';
    if (typeof workspaceId !== 'string') {
        throwNextTick(new InternalError("Expected value of _scrivito_workspace_id to be a string, but it is " + typeof workspaceId));
        return;
    }
    setCurrentWorkspaceId(workspaceId);
    if (workspaceId !== 'published') {
        editing_context_inPlaceEditingActive = params.hasOwnProperty('_scrivito_editing');
        editing_context_comparisonActive =
            !editing_context_inPlaceEditingActive && params.hasOwnProperty('_scrivito_comparison');
    }
}
// public API
...
function url_for_urlFor(target, options) {
    ...
    checkUrlFor.apply(void 0, [target, options].concat(excessArgs));
    var query;
    var hash;
    if (options) {
        query = options.query;
...
function setVisitorIdToken(token) {
    ...
    if (ui_adapter_uiAdapter)
        return;
    checkSetVisitorIdToken.apply(void 0, [token].concat(args));
    if (!provider) {
...
function navigateTo(target, options) {
    ...
    if (target === null) {
        nextTick(function () { return checkNavigateTo.apply(void 0, [target, options].concat(excessArgs)); });
        return;
    }
    checkNavigateTo.apply(void 0, [target, options].concat(excessArgs));
    var queryParameters;
    var hash;
    if (options) {
...
function navigateToPage(target) {
    ...
    var currentResource = browser_location_get();
...
function getRegisteredElementId(element) {
    for (var _i = 0, _a = Object.keys(element_registry_registry); _i < _a.length; _i++) {
        var key = _a[_i];
...
function withRegisteredElement(component) {
    ...
    return _a = /** @class */ (function (_super) {
            ...
            class_1.prototype.render = function () {
                return external_react_["createElement"](component, registered_element_assign({}, this.props, { registeredElement: this.registeredElement }));
            };
            return class_1;
        }(external_react_["Component"])),
        _a.displayName = display_name_from_component(component) + "WithRegisteredElement",
        _a;
}
function withContentZone(getContentReferenceFromProps, component) {
    ...
    return _a = /** @class */ (function (_super) {
            ...
            class_2.prototype.render = function () {
                return (external_react_["createElement"](ContentZoneContext.Provider, { value: this.registeredElement.id() }, external_react_["createElement"](component, registered_element_assign({}, this.props, { registeredElement: this.registeredElement }))));
            };
            return class_2;
        }(external_react_["Component"])),
        _a.displayName = display_name_from_component(component) + "WithContentZone",
...
/* harmony default export */ var option_marker = (connect(/** @class */ (function (_super) {
...
function validationResultsFor(model, attributeName) {
    ...
    checkValidationResultsFor.apply(void 0, [model, attributeName].concat(excessArgs));
...
function getValidationReportEntries(basicModel) {
    ...
    if (!validationsConfig)
        return [];
    var registry = getWindowRegistry();
    var content = wrap_in_app_class_wrapInAppClass(registry, basicModel);
    var reportEntries = [];
    validationsConfig.forEach(function (contentCallbackOrAttributeValidations) {
        reportEntries = reportEntries.concat((isContentValidationCallback(contentCallbackOrAttributeValidations)
...
function getReportEntriesForAttributeValidations(validations, content) {
    var attributeName = validations[0], callbacksOrConstraints = validations.slice(1);
    ...
    var basicField = realm_schema.basicFieldFor(content, attributeName);
    if (!basicField)
        return [];
    var appValue = wrap_in_app_class_wrapInAppClass(getWindowRegistry(), basicField.get());
    ...
    return Object(external_underscore_["flatten"])(callbacksOrConstraints.map(function (callbackOrConstraints) {
        ...
        if (isAttributeValidationConstraintsWithOptions(callbackOrConstraints)) {
            var validationOptions = callbackOrConstraints[0], constraints = callbackOrConstraints[1];
...
function formatWidgetValidationResults(basicWidget) {
    return getValidationReportEntries(basicWidget)
...
var WidgetTag = withContentZone(function (_a) {
    ...
    return {
        ...
        class_1.prototype.renderWithEditingMarker = function () {
            var _this = this;
            var widget = this.getWidget();
            var validationResultMessage = formatWidgetValidationResults(widget);
            return external_react_["createElement"](this.props.tag, widget_tag_assign({}, this.forwardedPropsWithMarker(), { ref: function (element) {
                    _this.props.registeredElement.set(element);
                    _this.widgetTagElement = element;
                }, className: this.className(), style: this.style(), onMouseOver: this.onMouseOver, onMouseOut: this.onMouseOut, children: [
                    external_react_["createElement"](editing_menu_marker, { key: "menuMarker", widget: widget, tooltip: validationResultMessage, setDragState: this.setDragState, onMouseOver: this.lockFocus, onMouseOut: this.unlockFocus })
                ].concat(external_react_["Children"].toArray(this.props.children), [
                    this.renderOptionMarker('top'),
                    this.renderOptionMarker('bottom'),
                ]) }, this.dataProps({
                displayValidationResult: !!validationResultMessage,
            })));
        };
        class_1.prototype.renderWithComparisonMarker = function (modification, placementModification) {
            ...
            return external_react_["createElement"](this.props.tag, widget_tag_assign({}, this.forwardedPropsWithMarker(), { ref: function (element) {
                ...
                ].concat(external_react_["Children"].toArray(this.props.children)) }, this.dataProps({
                modification: modification || placementModification,
            })));
        };
        class_1.prototype.renderWithoutMarker = function () {
            var _this = this;
            return external_react_["createElement"](this.props.tag, widget_tag_assign({}, this.forwardedProps(), { ref: function (element) {
                    _this.widgetTagElement = element;
                } }));
        };
        ...
        class_1.prototype.dataProps = function (_a) {
            var displayValidationResult = _a.displayValidationResult, modification = _a.modification;
            var dataProps = {
        ...
        class_1.prototype.style = function () {
            if (this.state.isDragging) {
                if (this.props.style) {
                    return widget_tag_assign({}, this.props.style, { opacity: 0.5 });
                }
                return { opacity: 0.5 };
            }
...
function ensureWidgetWrappedInWidgetTag(component) {
    var wrappedComponent = /** @class */ (function (_super) {
        provide_component_extends(class_1, _super);
        function class_1() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        class_1.prototype.render = function () {
...
function isComponentMissingName(component) {
    ...
    return (!component.displayName &&
        (!component.name ||
            ...
            component.name.startsWith('class_')));
}
...
function calculateCSSProperties(style, binaryToUrl) {
    if (Object(external_underscore_["isObject"])(style)) {
        ...
        return background_image_tag_assign({}, otherStyles, calculateBackgroundCSSProperties(background, binaryToUrl));
    }
    return {};
}
...
var link_tag_LinkTag = /** @class */ (function (_super) {
    ...
    LinkTag.prototype.render = function () {
        ...
        var linkProps = link_tag_assign({}, otherProps, { href: (to && this.hrefFor(to, params)) || '#', onClick: this.onClick, rel: this.rel(), target: this.target() });
...
function currentPageIsNotFound() {
    return loadWithDefaultIfOutdated(false, function () {
        var pageAndParams = getPageAndParams();
...
function getPageAndParamsFromAppState() {
    var state = loadableWithDefault(undefined, getCurrentPageState);
    if (state === undefined) {
        return undefined;
    }
    if (!isScrivitoPage(state)) {
...
/* harmony default export */ var child_list_tag = (connect((child_list_tag_a = /** @class */ (function (_super) {
        ...
        ChildListTag.prototype.render = function () {
            ...
            return (external_react_["createElement"](InPlaceEditingEnabledContextConsumer, null, function (isInPlaceEditingEnabled) {
                ...
                return external_react_["createElement"](tag, props, [
                    menuMarker
                ].concat(orderedChildren.map(function (child) { return (external_react_["createElement"](child_item, { key: child.id(), child: child, renderChild: renderChild })); })));
            }));
        };
...
var widget_content_AppWidgetWrapper = /** @class */ (function (_super) {
    ...
    AppWidgetWrapper.prototype.getAppWidgetProps = function () {
        ...
        if (this.props.widgetProps &&
            ...
            return widget_content_assign({}, appWidgetProps, widgetPropsWithoutWidget);
        }
        return widget_content_assign({}, appWidgetProps, this.props.widgetProps);
    };
    ...
    Editor.prototype._getContent = function () {
        ...
        return capture(function () {
            if (_this._field.type() === 'html') {
                ...
                return replaceInternalLinks(value);
            }
...
var content_tag_ContentTag = /** @class */ (function (_super) {
    ...
    ContentTag.prototype.render = function () {
        ...
        var field = realm_schema.basicFieldFor(content, attribute);
        if (!field) {
...
var current_page_extends = (undefined && undefined.__extends) || (function () {
    var extendStatics = function (d, b) {
        extendStatics = Object.setPrototypeOf ||
            ({ __proto__: [] } instanceof Array && function (d, b) { d.__proto__ = b; }) ||
            function (d, b) { for (var p in b) if (b.hasOwnProperty(p)) d[p] = b[p]; };
...
var current_page_CurrentPage = /** @class */ (function (_super) {
    current_page_extends(CurrentPage, _super);
    function CurrentPage() {
        return _super !== null && _super.apply(this, arguments) || this;
    }
    CurrentPage.prototype.render = function () {
        ...
        if (!pageComponent) {
            throw new ArgumentError("No component registered for obj class \"" + pageClass + "\"");
        }
        return external_react_["createElement"](pageComponent, { page: page, params: params });
    };
    return CurrentPage;
}(external_react_["Component"]));
...
var image_tag_ImageTag = /** @class */ (function (_super) {
    ...
    ImageTag.prototype.renderEmptyObj = function (attribute, obj, widthProp, htmlOptions) {
        return (external_react_["createElement"](content_tag, image_tag_assign({ attribute: attribute, content: obj, tag: 'img', src: image_placeholder, "data-scrivito-image-placeholder": true, width: widthProp }, htmlOptions)));
    };
...
function getBinary(content, attribute) {
    var schema = realm_schema.forInstance(content);
    if (!schema) {
        return;
    }
    var basicContent = unwrapAppClass(content);
    var definition = schema.attributes[attribute];
    if (!definition) {
...
function configureContentBrowser(configuration) {
    ...
    try {
        checkConfigure.apply(void 0, [configuration].concat(excessArgs));
    }
    catch (e) {
...
function getCustomComponentDimensions(element) {
    return get_custom_component_dimensions_getData(element).get() || null;
}
...
function getScrollPosition(element) {
    return get_scroll_position_getData(element).get() || null;
}
...
function getApplicationUrl() {
    return get_application_url_getData().get() || null;
}
function get_application_url_getData() {
    return new loadable_data({
        state: appState.subState('applicationUrl'),
        stream: new streamable_Streamable(function (subscriber) {
            var updateApplicationUrl = function () {
                return subscriber.next(getIncludingOrigin());
            };
            updateApplicationUrl();
            var observation = onCurrentPageChange(updateApplicationUrl);
            return function () { return observation.unsubscribe(); };
        }),
    });
}
...
function configure(configuration) {
    ...
    configure_checkConfigure.apply(void 0, [configuration].concat(excessArgs));
    if (isConfigured) {
...
function setAppAdapter(uiAdapterClient) {
    ...
    uiAdapterClient.setAppAdapter(getScrivitoVersion(), channel.port2);
}
...
var menu_patch_builder_MenuPatchBuilder = /** @class */ (function () {
    ...
    MenuPatchBuilder.prototype.insert = function (customMenuItem) {
        ...
        this.patch.insertIds = Object(external_underscore_["unique"])(this.patch.insertIds.concat([customMenuItem.id]));
        this.patch.items[customMenuItem.id] = menu_patch_builder_assign({}, Object(external_underscore_["pick"])(customMenuItem, 'description', 'group', 'icon', 'position', 'title'), Object(external_underscore_["mapObject"])(Object(external_underscore_["pick"])(customMenuItem, 'enabled'), function (v) { return !!v; }));
    };
    MenuPatchBuilder.prototype.modify = function (menuItem) {
        ...
        checkMenuModifyArguments.apply(void 0, [menuItem].concat(excessArgs));
        this.patch.modifyItems[menuItem.id] = menu_patch_builder_assign({}, this.patch.modifyItems[menuItem.id], Object(external_underscore_["pick"])(menuItem, 'group', 'icon', 'position', 'title'));
    };
...
function renderPage(page, render) {
    ...
    checkRenderPage.apply(void 0, [page, render].concat(excessArgs));
...
function preload(preloadDump) {
    ...
    checkPreload.apply(void 0, [preloadDump].concat(excessArgs));
